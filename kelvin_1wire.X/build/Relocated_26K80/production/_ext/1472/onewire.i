...
# 4 "/home/akhe/.mchp_packs/Microchip/PIC18F-K_DFP/1.3.84/xc8/pic/include/__size_t.h"
typedef unsigned size_t;

# 7 "/opt/microchip/xc8/v2.31/pic/include/c90/stdarg.h"
...
# 43 "/opt/microchip/xc8/v2.31/pic/include/c90/stdio.h"
struct __prbuf
{
char * ptr;
void (* func)(char);
...
# 29 "/opt/microchip/xc8/v2.31/pic/include/c90/errno.h"
extern int errno;

# 12 "/opt/microchip/xc8/v2.31/pic/include/c90/conio.h"
...
# 88 "/opt/microchip/xc8/v2.31/pic/include/c90/stdio.h"
...
# 30 "/opt/microchip/xc8/v2.31/pic/include/c90/math.h"
...
# 14 "/opt/microchip/xc8/v2.31/pic/include/c90/string.h"
...
# 34 "../../vscp-firmware/pic/common/inttypes.h"
...
output_low( pin );
_delay((unsigned long)((480)*(40000000/4000000.0)));
output_high( pin );
...
output_low( pin );
_delay((unsigned long)((2)*(40000000/4000000.0)));
input( pin );
...
_delay((unsigned long)((55)*(40000000/4000000.0)));
...
_delay((unsigned long)((120)*(40000000/4000000.0)));
}
...
temp = val_u8 >> i;
temp &= 0x01;
DS1820_WriteBit( pin, temp);
}

_delay((unsigned long)((105)*(40000000/4000000.0)));
}
...
bStatus = DS1820_Reset( pin );

if ( bStatus || bDoneFlagOneWire ) {
nLastDiscrepancyOnewire = 0;
return 0;
}


DS1820_WriteByte( pin, 0xF0 );
...
DS1820_Reset( pin );
DS1820_AddrDevice( pin, 0x55 );
DS1820_WriteByte( pin, 0x4E );
...
for ( i=0; i<76; i++ ) {
_delaywdt((unsigned long)((10)*(40000000/4000.0)));
}
...
# 561
temp = 0;
temp = (uint16_t)((uint16_t)scrpad[ 1 ] << 8);
temp |= (uint16_t)( scrpad[ 0 ] );

if ( 0x10 == romAddrOneWire[ 0 ] ) {
...
temp -= ( (uint16_t)0x100 >> 2 );


highres = scrpad[7] - scrpad[6];
highres = ((uint16_t)highres << 8);
if ( scrpad[ 7 ] ) {
highres = highres / (uint16_t)scrpad[ 7 ];
}
...
double DS1820_GetTempFloat( uint8_t pin )
{
return ( (float)DS1820_GetTempRaw( pin, 1 ) / (float)0x100 );
}

# 619
double DS1820_LateGetTempFloat( uint8_t pin )
{
return ( (float)DS1820_GetTempRaw( pin, 0 ) / (float)0x100 );
}
...
tPhy = (int8_t)(rawTemp/0x100 );



tPhyLow = rawTemp & 0xFF;

tPhyLow = tPhyLow * 100;
tPhyLow = (uint16_t)tPhyLow / 0x100;
